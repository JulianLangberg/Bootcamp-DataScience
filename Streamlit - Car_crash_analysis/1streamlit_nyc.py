...
def load_data():
    ...
    return df3,df_p, df_motivos_grouped_aux_pre, df_motivos_grouped_aux_post, df_motivos_grouped_aux_post_viernes, df_motivos_grouped_post_winter

df3,df_p, df_motivos_grouped_aux_pre, df_motivos_grouped_aux_post, df_motivos_grouped_aux_post_viernes, df_motivos_grouped_post_winter = load_data()
...
df['total_victims'] = df[['total_killed','total_injured',]].sum(axis=1)
...
def DateCovid(x):   
    global season
    if x <= pd.to_datetime('2020-01-01'):
         season = "1pre-Lockdown"
    elif x >= pd.to_datetime('2020-07-01'):
         season = "3post-Lockdown"
    #elif x > pd.to_datetime('2020-01-01') & x < pd.to_datetime('2020-07-01'):
    #     season = "Lockdown"
    else:
         season = "2Lockdown"
    return season

df_definitivo['lockdown']= pd.to_datetime(df_definitivo['crash_date']).apply(lambda x : DateCovid(x))
...
################ post covid por temporada (season)

############ lockdown vs lockdown-invierno

st.markdown("Los factores contributivos en Invierno también se mantienen, los menores accidentes son por la menor circulación.")
...
image = Image.open('Data-Science-1.jpg')
st.image(image)
...
st.title('Data Science - Forecasting')
st.markdown('Experimentation and Learning process')
image = Image.open('Proyeccion1.png')
st.image(image)

st.markdown('Insights confirmation')
image = Image.open('Proyeccion2.png')
st.image(image)
